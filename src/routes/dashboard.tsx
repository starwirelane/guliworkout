...
export default function Dashboard() {
  ...
  const [completed, setCompleted] = useState<Record<string, boolean>>(loadCompletions);
  ...
  useEffect(() => {
    if (notifGranted && profile && !notifScheduled.current) {
      scheduleNotification(profile.workout_time, profile.name);
      notifScheduled.current = true;
    }
  }, [notifGranted, profile]);
  ...
  const today: Day = plan.days[todayIndex];
  ...
  const orderedSessions = [mainSession, ...lightSessions].filter(Boolean);

  const { isTime, countdown, isPast } = getWorkoutStatus(profile.workout_time, now);
  ...
  const workoutTimeLabel = new Date(
    new Date().setHours(...(profile.workout_time.split(":").map(Number) as [number, number]))
      ...
      <main className="relative z-10 mx-auto max-w-7xl px-6 pb-20">
        <div className="mb-8 max-w-3xl">
          ...
          <h1 className="mt-2 serif text-4xl font-bold leading-[1.05] md:text-6xl">
            Hi {profile.name}, <span className="italic-accent italic">{today.title.toLowerCase()}</span>.
          </h1>
        ...
        {todayIndex >= 13 && allDone && (
          <div className="mt-10 rounded-3xl border-2 border-primary/30 bg-primary/10 p-8 text-center">
            ...
            <button onClick={() => navigate("/onboarding")}
              className="mt-5 rounded-full bg-primary px-7 py-3 font-semibold text-primary-foreground hover:scale-[1.02]">
